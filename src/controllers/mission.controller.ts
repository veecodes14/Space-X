...
//@route POST /api/v1/mission/request
//@desc Mission Request (user)
//@access Private
...
export const scheduleMission = async(req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if(!missionId || userId) {
            res.status(400).json({
                success: false,
                message: "MissionID and UserID are required"
            });
            return
        }
        ...
        if(!mission || mission.status !== 'pending') {
            ...
            mission.scheduledBy = userId as any
            await mission.save();

        res.status(200).json({
            success: true,
            messagr: "Mission scheduled successfully",
            data: mission
        })
